				...
				<ul>
					...
					<c:forEach var="i" begin="${groupStartNum}" end="${groupEndNum}">
						<c:choose>
							...
							<c:otherwise>
								<li>
									<a href="${pageContext.request.contextPath}/board?a=board&page=${i}&kwd=${kwd}">${i}</a>
								</li>
							</c:otherwise>
						</c:choose>
					</c:forEach>
					...
					<c:if test="${endPageNum > groupEndNum}">
						<li>
							<a href="${pageContext.request.contextPath}/board?a=board&page=${groupEndNum + 1}&kwd=${kwd}">▶</a>
						</li>
					</c:if>
				</ul>
			</div>
